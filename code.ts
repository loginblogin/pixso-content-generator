  ...
  await (async () => {
    ...
    async function loadAllFontsForNode(node: TextNode): Promise<void> {
      ...
      if (fonts.size === 0) {
        ...
        if (isFontName(fn)) fonts.set(fn.family + '|' + fn.style, fn)
      }
    ...
    if (imgNodesToFill.length > 0) {
      ...
      if (imgLoaded > 0) pixso.notify('Loaded ' + imgLoaded + ' image' + (imgLoaded > 1 ? 's' : ''))
    }
...
async function runAction_fill(target: SceneNode | null, notify: boolean): Promise<void> {
  isExecuting = true
  try {
